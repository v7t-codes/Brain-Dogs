...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath=u'C:\\Users\\Srujan\\Desktop\\Project4\\stroop_test_updated\\stroopex.psyexp',
...
target = visual.TextStim(win=win, name='target',
    ...
    font='Arial',
    pos=(0, 0), height=0.1, wrapWidth=None, ori=0, 
    color=1.0, colorSpace='hsv', opacity=1,
    depth=0.0);
...
while continueRoutine:
    ...
    # *key* updates
    if t >= 0.0 and key.status == NOT_STARTED:
        # keep track of start time/frame for later
        key.tStart = t
    ...
    if key.status == STARTED:
        ...
        if len(theseKeys) > 0:  # at least one key was pressed
            key.keys = theseKeys[-1]  # just the last key pressed
...
for thisTrial in trials:
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if t >= 0.5 and response.status == NOT_STARTED:
            ...
            event.clearEvents(eventType='keyboard')
        frameRemains = 0.5 + 1.7- win.monitorFramePeriod * 0.75  # most of one frame period left
